...
$deamonRunning = domoticz::deamonRunning();
...
echo '<label class="col-sm-4 control-label">{{Communication Domoticz}}</label>';
if (!$deamonRunning) {
...
<script>
    $('.bt_sendConfigToDEV').on('click', function () {
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            $('#div_alert').showAlert({message: '{{Vous pouvez maintenant envoyer le fichier log "domoticz" à jeedom.domoticz@kyuran.be}}', level: 'success'});
           // $('#ul_plugin .li_plugin[data-plugin_id=domoticz]').click();
        }
    });
    });


    function domoticz_postSaveConfiguration() {
        $('#ul_plugin .li_plugin[data-plugin_id=domoticz]').click();
    }
</script>
