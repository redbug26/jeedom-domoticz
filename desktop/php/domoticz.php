    ...
    <div class="col-lg-2">
        <div class="bs-sidebar">
            <ul id="ul_eqLogic" class="nav nav-list bs-sidenav">
                ...
                <?php
foreach (eqLogic::byType('domoticz') as $eqLogic) {
...
foreach ($eqLogics as $eqLogic) {
	...
	$image = "plugins/domoticz/doc/images/domoticz_icon.png";

	foreach ($eqLogic->getCmd() as $action) {

		if ($action->getConfiguration('type') == "core:ClosureState") {
			$image = sprintf("plugins/domoticz/doc/images/domoticz_icon.php?pos=%d", $action->execCmd());
		}
	}
...
<a style="display:none" class="btn btn-success btn-sm cmdAction" data-action="add"><i class="fa fa-plus-circle"></i> {{Ajouter une commande domoticz}}</a><br/>

<legend>Commandes</legend>
        <table id="table_cmd" class="table table-bordered table-condensed">
            <thead>
                <tr>
                    <th>{{Nom}}</th><th>{{commandName}}</th><th>{{Parameters}}</th><th>{{Options}}</th><th>{{Action}}</th>
                </tr>
            </thead>
            <tbody>
...
<?php include_file('desktop', 'domoticz', 'js', 'domoticz');?>
